...
export async function apiRequest(
  ...
  try {
    ...
    console.log(
      `🔵 API Request: ${method.toUpperCase()} ${API_URL}${endpoint}`,
    );
    ...
    if (
      ...
      console.error(
        `❌ API: Response is HTML instead of JSON for ${method.toUpperCase()} ${endpoint}`,
      );
      console.error(
    ...
    console.log(
      `✅ API Response successful: ${method.toUpperCase()} ${endpoint}`,
    );
    return responseData;
  } catch (error: any) {
    console.error(
      `❌ API Error for ${method.toUpperCase()} ${endpoint}:`,
      error.message,
    );
    throw error;
  }
  ...
  try {
    ...
    const mappedUsers = users.map((u: any) => {
      ...
      if (u.rol?.nombre) {
        ...
        console.log(
          `🔄 Mapeando usuario ${u.id}: rol original = "${u.rol.nombre}" -> normalizado = "${rolName}"`,
        );
...
export const getAdminArticles = async (): Promise<Article[]> => {
  ...
  try {
    ...
    if (!response) {
      console.log(
        "⚠️ Ningún endpoint especial funcionó, usando /articulos normal",
      );
    ...
    console.log(
      `✅ getAdminArticles: Encontrados ${articles.length} artículos`,
    );
...
export const getArchivedArticles = async (): Promise<Article[]> => {
  ...
  try {
    ...
    console.log(
      `✅ getArchivedArticles: Encontrados ${articles.length} artículos archivados`,
    );
    const mapped = articles.map(mapArticleFromAPI);
    return mapped;
  } catch (error) {
...
export const createArticle = async (
  ...
  console.log(
    "createArticle: Enviando payload:",
  ...
  console.log(
    "createArticle: Artículo mapeado:",
    JSON.stringify(mapArticleFromAPI(result), null, 2),
  );
